<script>
    ...
    export async function fetchAds() {
      ...
      try {
        const response = await fetch(`http://127.0.0.1:8000/ebay/?user_search=${encodeURIComponent(userSearch)}`);
        const data = await response.json();
        ads.set(data.ads);
      } catch (error) {
        console.error('Error fetching eBay ads:', error);
      } finally {
...
<h3 class="text-lg font-semibold text-center mt-6">Ebay Ads</h3>
{#if $ads.length > 0}
    ...
    {#each $ads as ad (ad.link)}
      <div class="ad bg-gray-100 rounded-lg p-4 shadow my-2">
        ...
        <div class="ad-info mt-2">
          ...
          <a href={ad.link} target="_blank" class="text-blue-500 hover:underline">View Listing</a>
        </div>
      </div>
    {/each}
  </div>
