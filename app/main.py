...
from app.services.eBay import get_ebay_ads
...
async def kijiji(user_search: str, user_location: str):
    ads = await retry_async(get_kijiji_ads, MAX_RETRIES, user_search, user_location)
    return {"ads": ads}


@app.get("/amazon/")
async def amazon(user_search: str, user_postal_code: str):
    if not valid_postal_code(user_postal_code):
        raise HTTPException(status_code=400, detail="Invalid Postal Code")
    ads = await retry_async(get_amazon_ads, MAX_RETRIES, user_search, user_postal_code)
    return {"ads": ads}


@app.get("/ebay/")
async def ebay(user_search: str, user_location: str):
    ads = await retry_async(get_ebay_ads, MAX_RETRIES, user_search, user_location)
    return {"ads": ads}
